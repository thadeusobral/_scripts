...
multipleENMs <- function(ocorrencia.file= "Taurea_var.txt", background.file= "Background_random.txt", cross_validation= 5){
...
back <- read.table(background.file, h=T)
ocor <- read.table(ocorrencia.file, h=T)


#cross_validation <- 5             #numero de repeticoes do cross validation
...
AOGCM0k <- paste("be_biovar_", j, "_0k_neotropic.txt", sep="")
AOGCM21k <- paste("be_biovar_", j, "_21k_neotropic.txt", sep="")
AOGCMrcp45 <- paste("be_biovar_", j, "_rcp45_neotropic.txt", sep="")

clima0k <- read.table(AOGCM0k, h=T)
clima21k <- read.table(AOGCM21k, h=T)
climarcp45 <- read.table(AOGCMrcp45, h=T)
...
Output0k <- cbind(Output0k, Bioclim= Bioclim.Pout0k.mean, Gower= Gower.Pout0k.mean, SVM= SVM.Pout0k.mean, GLM= GLM.Pout0k.mean, Maxent= Maxent.Pout0k.mean)
Output21k <- cbind(Output21k, Bioclim= Bioclim.Pout21k.mean, Gower= Gower.Pout21k.mean, SVM= SVM.Pout21k.mean, GLM= GLM.Pout21k.mean, Maxent= Maxent.Pout21k.mean)
Outputrcp45 <- cbind(Outputrcp45, Bioclim= Bioclim.Poutrcp45.mean, Gower= Gower.Poutrcp45.mean, SVM= SVM.Poutrcp45.mean, GLM= GLM.Poutrcp45.mean, Maxent= Maxent.Poutrcp45.mean)


} # fecha for 'j' - loop AOGCMs
...
write.table(Output0k, "Output_0k.txt", row.names= F, sep=" ")
write.table(Output21k, "Output_21k.txt", row.names= F, sep=" ")
write.table(Outputrcp45, "Output_rcp45.txt", row.names= F, sep=" ")
...
multipleENMs(ocorrencia.file= "Taurea_var.txt", background.file= "Background_random.txt", cross_validation= 5)




#######
### aspectos para melhorar o script

#	1. automatizar a media ponderada e salvar metricas de ajuste (AUC, TSS, ...)
#	2. transformar mapas em frequencias (ao inves de suitabilities)
#	3. salvar outputs detalhados por algoritmo e AOGCM, bem como o ensamble final
#	4. inserir varias especies ao mesmo tempo
#	5. implementar multi-cores (usar todos os processadores) para rodar vario modelos em paralelo
